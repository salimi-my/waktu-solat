<template>
  <TransitionRoot as="template" :show="enable">
    <Dialog as="div" class="relative z-10" @close="sendEnableEvent">
            ...
            <DialogPanel
              ...
              <div class="bg-white px-4 py-3 flex justify-center items-center">
                <button type="button"
                  ...
                  @click="sendEnableEvent" ref="cancelButtonRef">Tutup</button>
              </div>
            </DialogPanel>
          </TransitionChild>
        </div>
...
const emit = defineEmits(["enable-event"]);
const sendEnableEvent = () => emit("enable-event");
</script>